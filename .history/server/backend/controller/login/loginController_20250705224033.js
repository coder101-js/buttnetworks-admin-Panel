...
export const handlePassword = async (req, res) => {
  try {
    ...
    //fix handle password use id to find the user doce don't use global variable may cause unexpected  bug in the app
    console.log(password, "password");
    const isValid = await compare(password, user.password);
    if (!isValid) {
      res.status(403).send({ msg: "Incorrect Password", auth: false });
    } else {
      res.status(200).send({ msg: "Correct Password", auth: true });
    }
  } catch (err) {
    return res.status(500).send({ msg: err.message });
  }
