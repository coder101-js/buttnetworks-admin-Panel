...
export const handlePassword = async (req, res) => {
  try {
    ...
    if (!isValid) {
      ...
      const secret = (typeof process !== "undefined" && process.env && process.env.JWT_SECRET) ? process.env.JWT_SECRET : "supersecretkey"; // 🔒 keep it secret!

      // Create the token
      const token = jwt.sign(user, secret, {
        expiresIn: "1d", // ⏳ 1 day (you can use "1h", "7d", etc.)
      });
      const cookie = res.cookie("sessionId", token, {
